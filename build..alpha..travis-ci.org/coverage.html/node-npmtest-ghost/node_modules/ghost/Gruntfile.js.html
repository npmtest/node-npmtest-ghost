...
<html lang="en">
<head>
    <title>Code coverage report for node-npmtest-ghost/node_modules/ghost/Gruntfile.js</title>
    <meta charset="utf-8">
    <style>
        body, html {
            margin:0; padding: 0;
...
<div class="header low">
    ...
    <h1>Code coverage report for <span class="entity">node-npmtest-ghost/node_modules/ghost/Gruntfile.js</span></h1>
    <h2>
    ...
    <div class="path"><a href="../../../index.html">All files</a> &#187; <a href="index.html">node-npmtest-ghost/node_modules/ghost/</a> &#187; Gruntfile.js</div>
</div>
<div class="body">
...
<span class="cline-any cline-yes">1</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">// # Task automation for Ghost
//
...
var overrides      = require('./core/server/overrides'),
    ...
    path           = require('path'),
&nbsp;
    escapeChar     = process.platform.match(/^win/) ? <span class="branch-0 cbranch-no" title="branch not covered" >'^' </span>: '\\',
...
&nbsp;
    // ## Grunt configuration
&nbsp;
    configureGrunt = <span class="fstat-no" title="function not covered" >function (grunt) {</span>
        // #### Load all grunt tasks
        //
        // Find all of the task which start with `grunt-` and load them, rather than explicitly declaring them all
<span class="cstat-no" title="statement not covered" >        require('matchdep').filterDev(['grunt-*', '!grunt-cli']).forEach(grunt.loadNpmTasks);</span>
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        grunt.initConfig(cfg);</span>
&nbsp;
        // # Custom Tasks
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('help',</span>
...
<span class="cstat-no" title="statement not covered" >                console.log('Type `grunt --help` to get the details of available grunt tasks.');</span>
            });
&nbsp;
        // ### Documentation
        // Run `grunt docs` to generate annotated source code using the documentation described in the code comments.
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('docs', 'Generate Docs', ['docker']);</span>
&nbsp;
        // Runun `grunt watch-docs` to setup livereload &amp; watch whilst you're editing the docs
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('watch-docs', <span class="fstat-no" title="function not covered" >function () {</span></span>
<span class="cstat-no" title="statement not covered" >            grunt.config.merge({</span>
                watch: {
                    docs: {
...
&nbsp;
<span class="cstat-no" title="statement not covered" >            grunt.task.run('watch:docs');</span>
        });
&nbsp;
        // ## Testing
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('setTestEnv',</span>
            'Use "testing" Ghost config; unless we are running on travis (then show queries for debugging)',
<span class="fstat-no" title="function not covered" >            function () {</span>
<span class="cstat-no" title="statement not covered" >                process.env.NODE_ENV = process.env.TRAVIS ? process.env.NODE_ENV : 'testing';</span>
<span class="cstat-no" title="statement not covered" >                cfg.express.test.options.node_env = process.env.NODE_ENV;</span>
            });
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('ensureConfig', <span class="fstat-no" title="function not covered" >function () {</span></span>
<span class="cstat-no" title="statement not covered" >            var config = require('./core/server/config'),</span>
                done = this.async();
&nbsp;
<span class="cstat-no" title="statement not covered" >            if (!process.env.TEST_SUITE || process.env.TEST_SUITE !== 'client') {</span>
<span class="cstat-no" title="statement not covered" >                config.load().then(<span class="fstat-no" title="function not covered" >function () {</span></span>
...
<span class="cstat-no" title="statement not covered" >                    grunt.fail.fatal(err.stack);</span>
                });
            } else {
<span class="cstat-no" title="statement not covered" >                done();</span>
            }
...
&nbsp;
        ...
        // in a "new" state.
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('cleanDatabase', <span class="fstat-no" title="function not covered" >function () {</span></span>
...
&nbsp;
<span class="cstat-no" title="statement not covered" >            migration.reset().then(<span class="fstat-no" title="function not covered" >function () {</span></span>
<span class="cstat-no" title="statement not covered" >                models.init();</span>
<span class="cstat-no" title="statement not covered" >                return migration.init();</span>
...
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('test', 'Run a particular spec file from the core/test directory e.g. `grunt test:unit/apps_spec.js`', <span class="fstat-no" title="function not covered" >function (test) {</span></span>
<span class="cstat-no" title="statement not covered" >            if (!test) {</span>
<span class="cstat-no" title="statement not covered" >                grunt.fail.fatal('No test provided. `grunt test` expects a filename. e.g.: `grunt test:unit/apps_spec.js`. Did you mean `npm test` or `grunt validate`?');</span>
            }
&nbsp;
<span class="cstat-no" title="statement not covered" >            if (!test.match(/core\/test/) &amp;&amp; !test.match(/core\/server/)) {</span>
<span class="cstat-no" title="statement not covered" >                test = 'core/test/' + test;</span>
            }
&nbsp;
            // CASE: execute folder
<span class="cstat-no" title="statement not covered" >            if (!test.match(/.js/)) {</span>
<span class="cstat-no" title="statement not covered" >                test += '/**';</span>
            } else <span class="cstat-no" title="statement not covered" >if (!fs.existsSync(test)) {</span>
<span class="cstat-no" title="statement not covered" >                grunt.fail.fatal('This file does not exist!');</span>
            }
&nbsp;
<span class="cstat-no" title="statement not covered" >            cfg.mochacli.single.src = [test];</span>
<span class="cstat-no" title="statement not covered" >            grunt.initConfig(cfg);</span>
<span class="cstat-no" title="statement not covered" >            grunt.task.run('test-setup', 'mochacli:single');</span>
        });
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('stubClientFiles', <span class="fstat-no" title="function not covered" >function () {</span></span>
<span class="cstat-no" title="statement not covered" >            _.each(cfg.clientFiles, <span class="fstat-no" title="function not covered" >function (file) {</span></span>
<span class="cstat-no" title="statement not covered" >                var filePath = path.resolve(cwd + '/core/' + file);</span>
<span class="cstat-no" title="statement not covered" >                fs.ensureFileSync(filePath);</span>
            });
        });
&nbsp;
        // ### Validate
...
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('validate', 'Run tests and lint code', <span class="fstat-no" title="function not covered" >function () {</span></span>
<span class="cstat-no" title="statement not covered" >            if (process.env.TEST_SUITE === 'server') {</span>
<span class="cstat-no" title="statement not covered" >                grunt.task.run(['stubClientFiles', 'test-server']);</span>
            } else <span class="cstat-no" title="statement not covered" >if (process.env.TEST_SUITE === 'lint') {</span>
<span class="cstat-no" title="statement not covered" >                grunt.task.run(['lint']);</span>
            } else {
<span class="cstat-no" title="statement not covered" >                grunt.task.run(['validate-all']);</span>
            }
        });
&nbsp;
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('validate-all', 'Lint code and run all tests',</span>
            ['init', 'lint', 'test-all']);
&nbsp;
        // ### Test-All
        // **Main testing task**
        ...
        //
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('test-all', 'Run tests for both server and client',</span>
            ['test-server', 'test-client']);
&nbsp;
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('test-server', 'Run server tests',</span>
            ['test-routes', 'test-module', 'test-unit', 'test-integration']);
&nbsp;
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('test-client', 'Run client tests',</span>
            ['subgrunt:test']);
&nbsp;
        // ### Lint
        //
        // `grunt lint` will run the linter and the code style checker so you can make sure your code is pretty
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('lint', 'Run the code style checks and linter for server',</span>
            ['jshint', 'jscs']
        );
&nbsp;
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('lint-all', 'Run the code style checks and linter for server and client',</span>
            ['lint', 'subgrunt:lint']
        );
&nbsp;
        // ### test-setup *(utility)(
        // `grunt test-setup` will run all the setup tasks required for running tests
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('test-setup', 'Setup ready to run tests',</span>
...
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('test-unit', 'Run unit tests (mocha)',</span>
...
&nbsp;
        ...
        //
        // A coverage report can be generated for these tests using the `grunt test-coverage` task.
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('test-integration', 'Run integration tests (mocha + db access)',</span>
...
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('test-routes', 'Run functional route tests (mocha)',</span>
...
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('test-module', 'Run functional module tests (mocha)',</span>
...
&nbsp;
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('coverage', 'Generate unit and integration (mocha) tests coverage report',</span>
            ['test-setup', 'mocha_istanbul:coverage']
        );
&nbsp;
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('coverage-all', 'Generate unit and integration tests coverage report',</span>
...
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('master-warn',</span>
            'Outputs a warning to runners of grunt prod, that master shouldn\'t be used for live blogs',
<span class="fstat-no" title="function not covered" >            function () {</span>
<span class="cstat-no" title="statement not covered" >                console.log(chalk.red(</span>
                    'Use the ' + chalk.bold('stable') + ' branch for live blogs. '
                    + chalk.bold.underline('Never') + ' master!'
                ));
<span class="cstat-no" title="statement not covered" >                console.log('&gt;', 'Always two there are, no more, no less. A master and a ' + chalk.bold('stable') + '.');</span>
            });
&nbsp;
        // ## Building assets
        //
...
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('init', 'Prepare the project for development',</span>
...
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('build', 'Build client app',</span>
...
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('default', 'Build JS &amp; templates for development',</span>
            ['subgrunt:dev']);
&nbsp;
        // ### Production assets
...
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('prod', 'Build JS &amp; templates for production',</span>
            ['subgrunt:prod', 'uglify:prod', 'master-warn']);
&nbsp;
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('deps', 'Prepare dependencies',</span>
...
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('dev', 'Dev Mode; watch files and restart server on changes',</span>
...
<span class="cstat-no" title="statement not covered" >        grunt.registerTask('release',</span>
...
<span class="fstat-no" title="function not covered" >            function () {</span>
<span class="cstat-no" title="statement not covered" >                grunt.config.set('copy.release', {</span>
                    expand: true,
                    // #### Build File Patterns
                    ...
                    src: fs.readFileSync('.npmignore', 'utf8').split('\n').filter(Boolean).map(<span class="fstat-no" title="function not covered" >function (pattern) {</span>
<span class="cstat-no" title="statement not covered" >                        return pattern[0] === '!' ? pattern.substr(1) : '!' + pattern;</span>
                    }),
                    dest: '&lt;%= paths.releaseBuild %&gt;/'
                });
&nbsp;
<span class="cstat-no" title="statement not covered" >                grunt.task.run(['init', 'prod', 'clean:release', 'deps', 'copy:release', 'compress:release']);</span>
            }
        );
    };
&nbsp;
